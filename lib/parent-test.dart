  ...
  Widget _buildStatsRow(
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        ...
        children: [
          // Stat card for Overall GPA (matching the image's purple card)
          _StatCard(
            icon: Icons.auto_stories, // Book icon from image
            iconColor: const Color(0xFF7246D9), // Purple from image
            value: "3.8", // Static for now as GPA isn't in mockTests
            label: "Overall GPA",
          ),
          const SizedBox(width: 10), // Spacing between cards
          // Stat card for Class Rank (matching the image's orange card)
          _StatCard(
            icon: Icons.emoji_events, // Trophy icon from image
            iconColor: const Color(0xFFFFC107), // Orange from image
            value: "5th", // Static for now as rank isn't in mockTests
            label: "Class Rank",
          ),
          const SizedBox(width: 10), // Spacing between cards
          // Stat card for Attendance (matching the image's graph card)
          _StatCard(
            icon: Icons.show_chart, // Graph icon
            iconColor: Colors.indigo, // Example color
            value: "95%", // Static for now
            label: "Attendance",
          ),
          const SizedBox(width: 10), // Spacing between cards
          // Stat card for Total Tests (adapted from original 'Days to Next')
          _StatCard(
            icon: Icons.collections_bookmark, // A collection-like icon
            iconColor: Colors.teal, // Example color
            value: total.toString(),
            label: "Total Tests",
          ),
          const SizedBox(width: 10),

          // Stat card for Days to Next Test (adapted from original)
          _StatCard(
            icon: Icons.calendar_today, // Calendar icon
            iconColor: Colors.redAccent, // Example color
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: 100, // Adjusted width to match image appearance
      margin: const EdgeInsets.symmetric(horizontal: 5),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(12),
        border: Border(
          top: BorderSide(color: iconColor, width: 4),
        ), // Top border from image
        boxShadow: [
          BoxShadow(
            color: Colors.grey.withOpacity(0.1), // Softer shadow from image
            blurRadius: 10,
            offset: const Offset(0, 5),
          ),
        ],
      ...
      child: Column(
        crossAxisAlignment:
            CrossAxisAlignment.start, // Align text to start like image
        children: [
          Icon(icon, color: iconColor, size: 28), // Icon size from image
          const SizedBox(height: 8),
          Text(
            value,
            style: const TextStyle(
              fontWeight: FontWeight.w900,
              fontSize: 26, // Font size from image
              color: Color(0xFF333333),
            ),
          ),
          ...
          Text(
            ...
            style: TextStyle(
              ...
              fontSize: 11, // Font size from image
              fontWeight: FontWeight.w500,
            ),
            maxLines: 2,
